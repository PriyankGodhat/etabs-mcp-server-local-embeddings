...
Reports the frame joint forces for the point elements at each end of the
specified line elements
...
F1
    ...
    Type: [SystemDouble](https://docs.microsoft.com/dotnet/api/system.double)  
These is a one dimensional array that includes the joint force component in
the point element local 1-axis direction [F]

F2

    Type: [SystemDouble](https://docs.microsoft.com/dotnet/api/system.double)  
These is a one dimensional array that includes the joint force component in
the point element local 2-axis direction [F]

F3

    Type: [SystemDouble](https://docs.microsoft.com/dotnet/api/system.double)  
These is a one dimensional array that includes the joint force component in
the point element local 3-axis direction [F]
...
Copy
    ...
    Public Sub Example()
       ...
       'get frame joint forces for line object "1"
           ret = SapModel.Results.FrameJointForce("1", eItemTypeElm.ObjectElm, NumberResults, Obj, Elm, PointElm, LoadCase, StepType, StepNum, F1, F2, F3, M1, M2, M3)
